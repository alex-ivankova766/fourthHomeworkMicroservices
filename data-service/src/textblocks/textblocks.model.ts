...
export class TextBlock extends Model<TextBlock, BlockCreationAttrs> {
    ...
    @ApiProperty({example: 'main-hero-text', description: 'Unique title for the search'}) 
    ...
    @ApiProperty({example: 'JWT.IO allows you to decode, verify and generate JWT.', description: 'Text block content'}) 
    @Column( {type: DataType.STRING, allowNull: false})    
    content: string;
